...
class Homepage extends React.Component {
    ...
    renderInputField() {
      if(!this.state.blockNewMessages && (this.state.characterAsigned!="" || !this.state.blockNewMessages && (this.state.rolesAsigned < this.state.totalRoles))) {
        return (<div className="chatInput">
        <Input socket={this.state.socket} type="publicMessage" characterName={this.state.characterAsigned} />
      </div> );
      } else {
        return <div className="blockedInputMessage"><p>Wait...</p></div>;
      }
    }
