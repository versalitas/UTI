    ...
    componentDidMount() {
        ...
        socket.on('newMessage', function (data) {
            ...
            if (data.type == 'director') {
                if (
                    self.state.acotacions.length - 1 >
                    self.state.acotacions_count
                ) {
                    self.setState({
                        acotacions_count: self.state.acotacions_count + 1,
                        acotacion:
                            self.state.acotacions[self.state.acotacions_count],
                        acotacionsSceneCue:
                            self.state.acotacions.length -
                            (self.state.acotacions_count + 2),
                    });
                    console.log(
                        'anotation',
                        self.state.acotacions[self.state.acotacions_count]
                ...
                try {
                    if (data.acotacions.length > 0) {
                        self.setState({
                            ...
                            acotacionsSceneCue:
                                data.acotacions.length -
                                (self.state.acotacions_count + 1),
                        });
                        //self.setToTextArea();
                        console.log('set value to text area');
                        const elem = document.getElementById('textArea');
        ...
        socket.on('messageSceneCounter', function (data) {
            self.setState({ messageSceneCounter: data.messageSceneCounter });
        });
    }
        ...
        return (
            <div class="directorContainer">
                <aside className="directorSide">
                    ...
                    <br />
                    <hr />
                    <span className="containerCounterField">
                        Escena:{' '}
                        <span className="counterField">
                            <strong>
                                {sceneNumber + 1 + '/' + totalScene}
                            </strong>
                        </span>
                    </span>
                    <span className="containerCounterField">
                        Mensajes escena:{' '}
                        <span className="counterField">
                            <strong>{messageSceneCounter}</strong>
                        </span>
                    </span>
                    <span className="containerCounterField">
                        Acotaciones cola:{' '}
                        <span className="counterField">
                            <strong>{acotacionsSceneCue}</strong>
                        </span>
                    </span>
                    <span className="containerCounterField">
                        Personajes:{' '}
                        <span className="counterField">
                            <strong>{rolesAsigned + '/' + totalRoles}</strong>
                        </span>
                    </span>
                    <span className="containerCounterField">
                        Extras:{' '}
                        <span className="counterField">
                            <strong>{totalRolesExtra}</strong>
                        </span>
                    </span>
                    <div id="buttonContainer">
                        <span className="buttonField">
                            Siguiente Escena
                            <button onClick={this.changeToNextScene}>+</button>
                        </span>
                        <br />
                        <span className="buttonField">
                            Personaje extra
                            <button onClick={this.addOneExtra}>+</button>
                        </span>
                    </div>
                </aside>
