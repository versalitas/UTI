...
class Input extends React.Component {
    ...
    sendMessage = () =>{
      ...
      let textToSend = /*this.state.textarea.value*/elem.value.replace( /[^0-9a-zA-ZáéóíúàèìòùÀÈÌÒÙÁÉÍÓÚÜüÇçÑñ·ㄱ-힣+×÷=%♤♡☆♧)(*&^/~#@!-:;,&?`_|<>{}¥£€$◇■□●○•°※¤《》¡¿₩\[\]\"\' \\]/g , '').trim();
     ...
     render() {
       return (
            <form className="input" onSubmit={(e) => this.onSubmit(e)}>
                 {this.props.characterName!=""? (<label id="chatUser" htmlFor="chatUser">{this.props.characterName}:</label>):null}
                 <div className="inputArea" id="inputArea"><TextareaAutosize placeholder="Type..." id="textArea" ref="textareaChat" onKeyPress={(e) => { this.onChange(e) }} inputRef={tag => (this.state.textarea = tag)}/><button id="chatSendButton" type="submit"><img className="imageButton" src={utiArrow} alt="arrow icon"/></button></div>
            </form>
        )
    }
}
